...
S_train = 1:7200;      % snapshots for train
S_test = 1:3600;           % snapshots for test
EpL0 = 152;               % length of sensor time-series at full scale
EpB0 = 20;                % starting point of time-series at full scale
ROI = [1 88 1 88 22 22];  % first/last y, x, z (full space 88x88x44)
...
err_gen = @(Urecon) sqrt(mean(([U_test;V_test]-Urecon).^2, 'all'));
...
Urecon=(Psi_recon*SigmaU*PhiU')'+[Um;Vm];
